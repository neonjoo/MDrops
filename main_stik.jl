...
for i in 1:steps
    ...
    @time velocities = make_Vvecs_conjgrad_par(normals,faces, points, velocities_phys, 1e-6, 500)
    ...
    if false#(sum(marked_faces) >= minN_triangles_to_split) && (i - previous_i_when_split >= 5)
    ...
    if i % 1 == 0
        data = [points, faces, t, velocities_phys]
