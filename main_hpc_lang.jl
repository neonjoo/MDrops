...
Bm = parsed_args["Bm"]
w = parsed_args["w"]
elips = parsed_args["elips"] / 100

datadir = "/home/langins/sim_data/$elips"
println("Simulation: elipse $elips")


@load "./elips_$(elips)_perturbed.jl" data
...
# if simulation is continued, different parameters may be loaded below
H0 = [0., 0., 1.]
#w = 0.1
#Bm = 25.
mu = 10.
...
if continue_sim && isdir("$datadir")
	...
	cp("main_hpc_lang.jl", "$datadir/aa_source_code.jl"; force=true)
end
...
if !isdir("$datadir")
	...
	cp("main_hpc_lang_pert.jl", "$datadir/aa_source_code.jl"; force=true)
end
